...
- **Sample:** default analysis **n = 100** unique items after an **excluded calibration pilot**; stratify on assessment `item_map` axes (family, model, baseline/intervention, CEFR band/disagreement, truncation) with inclusion weights. **Correction (post-ship nits #16/#20):** an earlier shipped caveat said weights were conditional on the post-calibration frame; they are now **unconditional Horvitz–Thompson against the original pre-calibration pool** — \(\pi_i = n_s / N_s\), weight \(1/\pi_i\) (see `docs/human-eval.md`).
